...
public class UI {
	...
	public static void motraPartida(Partida partida, List<PecaXadrez> capturadas){
	...
	public static void printTabuleiro(PecaXadrez[][] pecas, boolean[][] possiviesMovimentos) {

		for (int i = 0; i < pecas.length; i++) {
			System.out.print((8 - i) + " ");
			for (int j = 0; j < pecas.length; j++) {
				printPeca(pecas[i][j], possiviesMovimentos[i][j]);
			}
			System.out.println();
		}
		System.out.println("   a b c d e f g h");
	...
	public static PosicaoNoXadrez lerPosicaoEscolhida(Scanner sc){
	 ...
	 private static void printPecasCapturadas(List<PecaXadrez> capturadas){
		 ...
		 System.out.println("Peçcas capturadas");
